...
Hinge 0.1.0 — DEVELOPMENT PREVIEW
...
Motion studio includes three presets, one saved personal setup, and calibration.
Hinge is based on MacDuo. See LICENSE and ATTRIBUTION.md for credits.
TEXT
codesign --verify --strict "$STAGE/Hinge.app"
hdiutil create -volname 'Hinge Preview' -srcfolder "$STAGE" -format UDZO -ov build/Hinge-0.1.0-preview.dmg
hdiutil verify build/Hinge-0.1.0-preview.dmg
shasum -a 256 build/Hinge-0.1.0-preview.dmg > build/Hinge-0.1.0-preview.dmg.sha256
